"""
Day 18: Residual analysis for the Day 17 ensemble — breaks down error
by store and by month to find where the model underperforms.
"""
import numpy as np
import pandas as pd
from pathlib import Path
from lightgbm import LGBMRegressor
from catboost import CatBoostRegressor

from train_lightgbm import load_data, split_features_target, evaluate

KEEP_FEATURES = [
    "price", "revenue", "daily_sales_lag_14", "daily_sales_lag_7",
    "daily_rolling_mean_7", "daily_rolling_std_7", "customer_segments",
    "seasonality_factors", "demand_trend", "promotions",
]

LGBM_PARAMS = {
    "n_estimators": 653, "learning_rate": 0.026, "num_leaves": 117,
    "max_depth": 11, "min_child_samples": 9, "random_state": 42,
}

ENSEMBLE_WEIGHT = 0.5  # Day 17's best blend: 50/50


def main():
    print("Loading train/test data ...")
    train, test = load_data()
    X_train, y_train, cat_cols = split_features_target(train)
    X_test, y_test, _ = split_features_target(test)
    for col in cat_cols:
        X_test[col] = pd.Categorical(X_test[col], categories=X_train[col].cat.categories)

    X_train_r = X_train[[c for c in KEEP_FEATURES if c in X_train.columns]]
    X_test_r = X_test[[c for c in KEEP_FEATURES if c in X_test.columns]]
    cat_cols_r = [c for c in cat_cols if c in X_train_r.columns]

    print("Training LightGBM ...")
    lgbm = LGBMRegressor(**LGBM_PARAMS)
    lgbm.fit(X_train_r, y_train, categorical_feature=cat_cols_r)
    preds_lgbm = lgbm.predict(X_test_r)

    print("Training CatBoost ...")
    cat_idx = [X_train_r.columns.get_loc(c) for c in cat_cols_r]
    cb = CatBoostRegressor(iterations=653, learning_rate=0.026, depth=10, random_state=42, verbose=False)
    cb.fit(X_train_r, y_train, cat_features=cat_idx)
    preds_cb = cb.predict(X_test_r)

    preds_ensemble = ENSEMBLE_WEIGHT * preds_lgbm + (1 - ENSEMBLE_WEIGHT) * preds_cb

    overall = evaluate(y_test.values, preds_ensemble)
    print(f"\nOverall ensemble WAPE: {overall['WAPE (%)']:.3f}%")

    # Build a results dataframe with identifying columns from the original test set
    results = test.reset_index(drop=True).copy()
    results["y_true"] = y_test.values
    results["y_pred"] = preds_ensemble
    results["residual"] = results["y_true"] - results["y_pred"]
    results["abs_error"] = results["residual"].abs()
    results["month"] = pd.to_datetime(results["date"]).dt.to_period("M")

    # --- Residual distribution ---
    print("\n--- Residual distribution ---")
    print(f"Mean residual: {results['residual'].mean():.3f}  (positive = model under-predicts on average)")
    print(f"Std of residual: {results['residual'].std():.3f}")
    print(f"Skew: {results['residual'].skew():.3f}")

    # --- By store ---
    print("\n--- WAPE by store (worst 10) ---")
    def wape_group(g):
        return (g["abs_error"].sum() / g["y_true"].sum()) * 100

    by_store = results.groupby("store_id").apply(wape_group, include_groups=False).sort_values(ascending=False)
    print(by_store.head(10).to_string())
    print(f"\nBest 5 stores:\n{by_store.tail(5).to_string()}")

    # --- By month ---
    print("\n--- WAPE by month ---")
    by_month = results.groupby("month").apply(wape_group, include_groups=False).sort_values(ascending=False)
    print(by_month.to_string())


if __name__ == "__main__":
    main()